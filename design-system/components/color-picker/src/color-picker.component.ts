...
import {
    getHexString as getHexConv,
    parseColor as parseColorConv,
} from '@components/color-picker/src/color-converter.js';
...
import { TsVisuallyHidden } from '@components/visually-hidden/index.js';
...
export default class TsColorPickerComponent extends ComponentElement implements ComponentFormControl {
    ...
    private emitColorChange() {
        ...
        this.dispatchEvent(new CustomEvent('ts-change', { bubbles: true, cancelable: false, composed: true, detail }));
        this.dispatchEvent(new CustomEvent('ts-input', { bubbles: true, cancelable: false, composed: true, detail }));
    }

    private emitColorInput() {
        const detail = { value: this.value };
        this.dispatchEvent(new CustomEvent('ts-input', { bubbles: true, cancelable: false, composed: true, detail }));
    }

    private emitColorChangeOnly() {
        const detail = { value: this.value };
        this.dispatchEvent(new CustomEvent('ts-change', { bubbles: true, cancelable: false, composed: true, detail }));
    }
