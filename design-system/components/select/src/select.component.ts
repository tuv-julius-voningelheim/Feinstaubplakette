...
import { TsTag } from '@components/tag/index.js';
...
export default class TsSelectComponent extends ComponentElement implements ComponentFormControl {
    ...
    /**
     * Makes the select readonly. Applies a locked visual style. The dropdown can still be opened
    ...
    private handleDocumentKeyDown = (event: KeyboardEvent) => {
        ...
        if (['ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
            ...
            // Bail out if there are no enabled options to navigate to
            ...
            if (!this.open) {
                ...
                // If an enabled option is already selected, stop here because we want that one to remain highlighted
                // when the listbox opens for the first time
            ...
            if (event.key === 'ArrowDown') {
                // Find the next enabled option after currentIndex (wrapping around)
                newIndex = this.findNextEnabledOptionIndex(allOptions, currentIndex, 1);
            } else if (event.key === 'ArrowUp') {
                // Find the previous enabled option before currentIndex (wrapping around)
                newIndex = this.findNextEnabledOptionIndex(allOptions, currentIndex, -1);
            } else if (event.key === 'Home') {
                const firstEnabled = enabledOptions[0];
                if (firstEnabled) {
                    newIndex = allOptions.indexOf(firstEnabled);
                }
            } else if (event.key === 'End') {
                const lastEnabled = enabledOptions[enabledOptions.length - 1];
                if (lastEnabled) {
                    newIndex = allOptions.indexOf(lastEnabled);
                }
            }
    ...
    private getFirstOption() {
        return this.querySelector<TsOption>('ts-option:not([disabled])') ?? this.querySelector<TsOption>('ts-option');
    }

    // Finds the next/previous enabled option index relative to currentIndex, wrapping around. The direction must be
    // either 1 (forward) or -1 (backward). Returns the original currentIndex when no enabled option is found.
    private findNextEnabledOptionIndex(allOptions: TsOption[], currentIndex: number, direction: 1 | -1) {
        const total = allOptions.length;
        if (total === 0) {
            return -1;
        }

        // When there is no current option, start the search from the first/last option depending on direction
        let nextIndex = currentIndex < 0 ? (direction === 1 ? -1 : 0) : currentIndex;

        for (let step = 1; step <= total; step++) {
            nextIndex = (nextIndex + direction + total) % total;
            const candidate = allOptions[nextIndex];
                        ...
                        >
                            ...
                            ${
                                this.readonly && this.lock && !this.disabled
                                    ? html`
                                          <span class="select__clear select__lock-icon" aria-hidden="true">
                                              <ts-icon
                                                  name="lock"
                                                  library="system"
                                                  size="20"
